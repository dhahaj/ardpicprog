/*
   Ardpicprog sketch
   https://rweather.github.io/ardpicprog/
*/
...
struct deviceInfo
{
  ...
  const int16_t  deviceId;      // Device ID for the PIC (-1 if no id).
  const uint32_t programSize;   // Size of program memory (words).
  const uint32_t configStart;   // Flat address start of configuration memory.
  const uint32_t dataStart;     // Flat address start of EEPROM data memory.
  const uint16_t configSize;    // Number of configuration words.
  const uint16_t dataSize;      // Size of EEPROM data memory (bytes).
  const uint16_t reservedWords; // Reserved program words (e.g. for OSCCAL).
  const uint16_t configSave;    // Bits in config word to be saved.
  const uint8_t  progFlashType; // Type of flash for program memory.
  const uint8_t  dataFlashType; // Type of flash for data memory.
};
...
void initDevice(const struct deviceInfo *dev) {
  ...
  s << endl;
  s << F("ProgramRange: 0000-");
  printHex8(programEnd);
  s << endl << F("ConfigRange: ");
...
void cmdDevices(const char *args) {
  ...
  int index = 0;
  for (;;) {
    const char *name = (const char *) (pgm_read_word(&(devices[index].name)));
    if (!name)
      break;
    if (index > 0) {
      s << F(",");
...
void cmdHelp(const char *args) {
  ...
  for (;;) {
    ...
    if (!name)
      break;
    const char *desc = (const char *) (pgm_read_word(&(commands[index].desc)));
    const char *args = (const char *) (pgm_read_word(&(commands[index].args)));
    printProgString(name);
    if (args) {
      s << (' ');
